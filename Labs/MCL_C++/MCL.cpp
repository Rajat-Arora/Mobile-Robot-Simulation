...
// Position of Landmarks wrt the world frame.
...
// Global Functions to take modulus and generate real numbers.
...
class Robot{
	...
	/*
   	* @brief Constructor to initialize the Pose{x,y,orient} to random values
   	*    and set noise to 0.
   	*/
	Robot(){
	...
	/*
   	* @brief Set given x,y,orient position.
   	*    Check the values if in range of world then set to class variables.
   	*/
	...
	/*
   	* @brief Measure the distance from landmarks.
   	*    Return vector of distances of robot pose from landmarks.
   	*/
	vector<double> sense()
    {
	...
	/*
   	* @brief Change turn and forward acoording to kinematics.
   	*    Cyclic truncate the x,y and orient and also simulate noise.
   	*/
...
//--------------------------Print readings and position of landmarks------------------------------------
	string show_pose()
    {
...
//--------------------------------------------------------------------------------------------------------
	...
	/*
   	* @brief Used to set weight.
   	*/
	double measurement_prob(vector<double> measurement) //measurement is sensed distance of thr robot from landmark.
    {      
        // Calculates how likely a measurement should be
        double prob = 1.0;
        double dist;

        //dist is the distnace between the particle and landmark
		// Taking dist as mu and measurement[i] and x in mu-x of gaussian dist formula.
        for (int i = 0; i < sizeof(landmarks) / sizeof(landmarks[0]); i++) {
            dist = sqrt(pow((x - landmarks[i][0]), 2) + pow((y - landmarks[i][1]), 2));
